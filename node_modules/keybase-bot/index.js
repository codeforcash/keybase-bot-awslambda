...
async function keybaseStatus(workingDir, homeDir) {
  const status = await keybaseExec(workingDir, homeDir, ['status', '--json'], {
   ...
   if(status) {
    ...
    throw new Error('Failed to get current username and device name.' + JSON.stringify(status));
  }
}
  ...
  async init(username, paperkey, options) {
    ...
    try {
      ...
      if (this.username !== username) {
        throw new Error('Failed to initialize service.' + 'this.username: ' + this.username + ' ; username: ' + username + ' ; currentInfo: ' + JSON.stringify(currentInfo));
      }
    } catch (err) {
...
class Bot {
  ...
  async _prepWorkingDir() {
    const keybaseBinaryLocation = process.env['LAMBDA_TASK_ROOT'] + '/gopath/bin/keybase';
